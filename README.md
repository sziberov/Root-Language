...
- Ключевое слово `final` (вместо `const`), для объявления констант более широкого спектра
...
<Переменная> = <Идентификатор>[':' <Тип>][ '=' <Выражение>][',' <Переменная>]
```
