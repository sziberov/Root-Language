...
- Многофункциональный интерпретатор
   ...
   - Необязательнные метки параметров функций при вызове
...
- Расширенная типизация
   ...
   - Выражения для всех композитов, а не только функций-замыканий
   - Возможность хранения функций в массивах
...
Отличие от общепринятой реализации состоит в том, что каждый композит имеет свой собственный список ретейнеров, который наполняется и опустошается за время выполнения кода. Использование такого списка позволяет динамически отслеживать и уведомлять о недоступности композитов, исключая из выборки ретейн-циклы, чего не может предоставить счётчик в виде обычного числа.

Композит А входит в список ретейнеров композита Б, если он использует того как свою область видимости, или ссылается на него как минимум в одном из своих импортов, мемберов или наблюдателей. То же самое условие, только наоборот, относится к уходу из списка - отсутствие связей значит отсутствие нужды в ретейне.

Попытки очистки памяти происходят в трёх случаях:
- После удаления ссылки из бывшего ретейнера (в т.ч. автоматического).
- После выполнения инструкции, результатом которой стал композит.
- В момент возврата из тела функции (в т.ч. вложенного).

Автоматическое (не принудительное) решение об удалении композита принимается только в том случае, если композит недоступен ни глобально, ни функционально. Функциональная доступность означает прямое равенство или ссылаемость на композит из предыдущей области видимости в стеке вызовов или из текущего значения неглобального возврата. Проверка с предыдущей ОВ используется для того чтобы предотвратить досрочное удаление оной вместе с текущей, которая на неё ссылкается, но сама уже недоступна. ЗВ устанавливается сразу после выполнения завершающей инструкции и перед проверками доступности, что впоследствии позволяет сохранить в памяти (по крайней мере до конца выполнения следующей инструкции) и его, и ОВ завершающейся функции, если он на неё ссылается.

#### Наследование

Каждый композит, кроме глобального, может иметь своего родителя и обязательно имеет область видимости. Свойство `Родитель` используется для сравнения типа композита, а `Область видимости` для поиска мемберов, в случае их ненахождения в текущем композите. Область видимости может устанавливаться динамически в моменты вызова функций, для переключения между статичным контекстом и контекстом объекта. Количество объектов при инициализации композита, наследуемого от другого композита, не ограничивается одним, что позволяет разделить области видимости таким образом, чтобы функции родителя не могли случайно обратиться к дочерним мемберам, а также не было препятствий перегрузкам. Процесс инициализации начинается с создания всей цепочки объектов и назначения им функциональных мемберов, после чего, от исходного до производного, вызываются инициализаторы композитов. Предварительное назначение всех функций необходимо для поддержки виртуальных оных, где поиск должен выполняться с новейших перегрузок.
...
Объект А
    ...
    Область видимости == Класс А
    Мемберы
        Суб.А == 4                         Находится в объекте Б
        Суб.Б == 5                         Находится в объекте Б
        Суб.В == Функция 6                 Находится в объекте Б ещё до его инициализации
        Суб.Г == 7                         Находится в объекте Б
        А == 1                             Инициализирован классом А
        Б == Функция 2                     Инициализирован из класса А, область видимости при вызове == Объект А
        В == Функция 3 (виртуально)        Инициализирован из класса А, новейшая замена == Суб.В

Объект Б
    Родитель == Класс Б
    Область видимости == Объект А
    Мемберы
        Супер.А == 1                       Находится в объекте А
        Супер.Б == Функция 2               Находится в объекте А
        Супер.В == функция 3 (виртуально)  Находится в объекте А, новейшая замена == В
        А == 4                             Инициализирован классом Б
        Б == 5                             Инициализирован классом Б
        В == Функция 6                     Инициализирован из класса Б, область видимости при вызове == Объект Б
        Г == 7                             Инициализирован классом Б
```