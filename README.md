...
- После удаления ссылки из бывшего ретейнера.
- После выполнения инструкции, результатом которой стал композит.
- В момент возврата из функции (проверке подвергается её область видимости).

Автоматическое решение об удалении композита принимается только в том случае, если композит недоступен ни глобально, ни функционально. Функциональная доступность означает прямое равенство или ссылаемость на композит из предыдущей области видимости в стеке вызовов или из текущего значения неглобального возврата. Значение возврата устанавливается сразу после выполнения завершающей инструкции и перед проверками доступности, что впоследствии позволяет сохранить в памяти (по крайней мере до конца выполнения следующей инструкции) и его, и ОВ завершающейся функции, если он на неё ссылается. Само же временное хранение необходимо для правильной обработки всех вложенных и прилегающих выражений, которые могут быть в инструкции (высшем конструкте кода).
