...
### Идеологические преимущества по сравнению со Swift:
...
- Более удобное управление памятью
   - Автоматическое остлеживание доступности композитов и ретейн-циклов (ARC, учитывающий каждого ретейнера отдельно)
...
Объявления подразделяются на 4 группы: импорты, операторы, мемберы и наблюдатели. Объявление обрабатывается раньше другого кода, а его значение считывается в разных местах по идентификатору. Идентификаторы каждого из объявлений, находящихся в одной группе, обязаны быть уникальными. Исключение составляют мемберы-композиты с дженерик-параметрами и функции, где поиск возможно осуществлять по типам и количеству параметров. У объявлений могут быть модификаторы, влияющие на внешний доступ к ним, или несущие другие эффекты.
