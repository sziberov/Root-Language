...
struct PrimitiveType : Type {
	...
	TypeRef operator+() const override {
		return Ref<PrimitiveType>(operator double());
	}
...
const TypeRef PredefinedPTypeTypeRef = Ref<PredefinedType>(PredefinedTypeID::PType, [](const TypeRef& type) {
	return type->ID != TypeID::Undefined && !type->concrete;
});
...
int main() {
	...
	TypeRef nillableType = Ref<NillableType>(parenType);
	...
	cout << "(int?)? normalized: " << nillableType->normalized()->toString() << "\n";
	*/
